...
int levantar_bitmap(char *path)
{
    ...
    if (file == NULL)
    {
        ...
        if (file == NULL)
        {
            log_error(logger, "[FILE_SYSTEM]: ERROR AL CREAR BITMAP");
            return FAILURE;
        }
        int fd = fileno(file);
        ftruncate(fd, size);
...
void terminar_ejecucion()
{
    ...
    log_info(logger, "[FILE_SYSTEM]: CONFIG FINALIZADO.");
    config_destroy(config_superbloque);
    log_info(logger, "[FILE_SYSTEM]: CONFIG SUPERBLOQUE FINALIZADO.");
    free(bitarray);
    log_info(logger, "[FILE_SYSTEM]: BITARRAY FINALIZADO.");
    free(bitmap);
    log_info(logger, "[FILE_SYSTEM]: BITMAP FINALIZADO.");
    log_destroy(logger);
}
